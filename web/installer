...
function checkPlatform($quiet)
{
    ...
    if (ini_get('xdebug.show_exception_trace')) {
        $warnings['xdebug_exception'] = true;
    }

    if (ini_get('xdebug.scream')) {
        $warnings['xdebug_scream'] = true;
    }
        ...
        foreach ($warnings as $warning => $current) {
            switch ($warning) {
                ...
                case 'xdebug_scream':
                    ...
                    $text .= "Add the following to the end of your `php.ini`:".PHP_EOL;
                    $text .= "    xdebug.scream = Off";
                    break;

                case 'xdebug_exception':
                    ...
                    $text .= "Add the following to the end of your `php.ini`:".PHP_EOL;
                    $text .= "    xdebug.show_exception_trace = Off";
                    break;
...
function installComposer($installDir, $quiet)
{
    ...
    while ($retries--) {
        ...
        copy($source, $file, getStreamContext());
        try {
